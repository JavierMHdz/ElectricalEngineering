...
vector<Net *>* Design::toposort(){   
   ...
   for(it = designNets.begin(); it != designNets.end(); ++it){
     if(colormap[it->first] == 0){
         //to check if it's only an output
         for(unsigned int i = 0; i < pos.size(); ++i){
            if((it->second->name()).compare(pos[i]) == 0){
...
*/
   ...
   vector<Gate*> *drivers = n->getDrivers();


    for(unsigned int i = 0; i < drivers->size(); ++i){
         vector<Net*> *netInputs = ((*drivers)[i])->getInputs();
         for(unsigned int i = 0; i < netInputs->size(); ++i){
            if(colormap[((*netInputs)[i])->name()] == 0){
               dfs_visit(((*netInputs)[i]),flist, colormap);
            }
         }
         delete netInputs;
    }   
